...
enum RemoteUIParamType{
	...
	REMOTEUI_PARAM_INT = 101,
	REMOTEUI_PARAM_BOOL = 102,
	REMOTEUI_PARAM_STRING = 103
};
...
class ofxRemoteUI{
	...
	protected:
		...
		vector<string> scanForUpdatedParams();

		void updateParamsInList(vector<string>paramsPendingUpdate);
		bool hasParamChanged(RemoteUIParam p);

		void updateParamFromDecodedMessage(ofxOscMessage m, DecodedMessage dm);
		void syncParamToPinter(string paramName);
